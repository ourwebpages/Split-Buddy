...
export function memberLabel(group, userId) {
  ...
  if (group?.memberNames?.[userId]) return group.memberNames[userId];
  // Fall back to a readable shortened UID rather than raw bytes
  return `Member (${String(userId).slice(0, 6)})`;
}
