...
import { computeBalances, simplifyDebts, EPSILON } from '../lib/calculations';
import { memberLabel } from '../lib/groupService';

export default function BalanceCard({ group, expenses }) {
  const memberIds = useMemo(() => group?.members || [], [group?.members]);
  ...
  const allSettled = Object.values(balances).every((b) => Math.abs(b) <= EPSILON);
      ...
      <div className="p-5">
        ...
        <div className="grid gap-2.5 sm:grid-cols-2">
          {memberIds.map((userId) => {
            ...
            const name = memberLabel(group, userId);
            const initial = name.charAt(0).toUpperCase() || 'U';
            const isOwed = balance > EPSILON;
            const owes = balance < -EPSILON;
        ...
        <div className="mt-5 rounded-xl border border-gray-200/90 bg-gradient-to-br from-gray-50 to-white p-4">
          ...
          {allSettled ? (
            <div className="mt-3 flex items-center gap-2 rounded-lg bg-emerald-50 p-2.5 text-xs font-medium text-emerald-800">
              <span>Everyone is settled up! No payments needed right now.</span>
            </div>
          ) : (
